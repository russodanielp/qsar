...
def main():
    ...
    # Script outline 12/30/16
    # 1. Read in FP matrix (containing only rel fps( & profile
    # 2. Concatenate profile to the predicted profile
    # 3. Many compounds probably dont have FPs, so first start on compounds that have data
    # 4. Concatenate FPs to bio profile and try
    profile = DS.profile_3.load()
    fp_matrix_0 = DS.fp_matrix_0.load()
    cmps_to_remove = [cmp for cmp in profile.profile.index if cmp not in fp_matrix_0.profile.index]

    profile = profile.remove_cmps(cmps_to_remove)

    print(fp_matrix_0 + profile)
